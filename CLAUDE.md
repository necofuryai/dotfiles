...
- Files at the repo root deploy into `$HOME` unless listed in `.chezmoiignore`
  (README.md, LICENSE, THIRD-PARTY-LICENSES.md, `docs/` and this file are
  ignored there). Keep it that way when adding repo-level docs. Entries whose
  names start with `.` (`.agents/`, `.claude/`, `.github/`, `.gitleaks.toml`, …)
  never deploy — chezmoi skips them in the source dir — so they need no ignore
  line. The exception is chezmoi's own `.chezmoi*` entries: `.chezmoiscripts/`
  runs scripts and `.chezmoiexternal.*` creates targets. `.chezmoiignore`
  patterns match target paths: a `.agents` line would hide `~/.agents` from
  `chezmoi unmanaged`, not the repo's `.agents/`.
